...
void SpeedreaderToolbarDataHandlerImpl::OnThemeChanged() {
  ...
  if (base::FeatureList::IsEnabled(features::kBravePaddedWebContent)) {
    // The border is rendered in HTML. In padded content mode, hide the border
    // by giving it the same color as the background. When this feature flag is
    // removed, consider removing the border in HTML.
    colors->border =
        color_provider->GetColor(kColorSpeedreaderToolbarBackground);
  }
  colors->button_hover =
