...
constexpr char kTopicsResponse[] = R"([
  ...
  {
    "topic_index": 2,
    "title": "More than 2,000 people killed as earthquake strikes western Afghanistan",
    "claude_title": "More than 2,000 people killed as earthquake strikes western afghanistan",
...
TEST_F(TopicsFetcherTest, TopicsAreJoinedAndParsedCorrectly) {
  ...
  EXPECT_EQ(base::Time::FromJsTime(1696733841352), israel.timestamp);
  EXPECT_EQ(7.8989222141, israel.overall_score);
  EXPECT_EQ(1.7777073399, israel.breaking_score);
  EXPECT_EQ(10u, israel.articles.size());

  auto afghanistan = topics[1];
  EXPECT_EQ("Afghanistan earthquake", afghanistan.claude_title_short);
  EXPECT_EQ(1u, afghanistan.articles.size());
  auto afghanistan_article = afghanistan.articles[0];
  ...
  EXPECT_EQ(GURL("https://news.sky.com/story/"
                 "earthquake-in-afghanistan-leaves-at-least-15-dead-and-40-"
                 "injured-12978962"),
            afghanistan_article.url);
  EXPECT_EQ(GURL("https://e3.365dm.com/23/10/1600x900/"
  ...
  EXPECT_EQ(base::Time::FromJsTime(1696675740000),
            afghanistan_article.publish_time);
