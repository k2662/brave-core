...
type GetTokenBalancesForAddressAndChainIdArg = {
  ...
  tokens: GetBlockchainTokenIdArg[]
  chainId: string
  coin: typeof CoinTypes.ETH | typeof CoinTypes.FIL | typeof CoinTypes.BTC
}
                    ...
                    async (accountId: BraveWallet.AccountId) => {
                      ...
                      if (ankrSupportedAccountNetworks.length) {
                        const { balances, errorMessage } =
                          await jsonRpcService.ankrGetAccountBalance(
                            accountId.address,
...
async function getSelectedNetwork(
  ...
  return (
    (await braveWalletService.getNetworkForSelectedAccountOnActiveOrigin())
      .network ?? undefined
  )
}
