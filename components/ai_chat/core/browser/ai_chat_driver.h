...
#ifndef BRAVE_COMPONENTS_AI_CHAT_CORE_BROWSER_AI_CHAT_DRIVER_H_
#define BRAVE_COMPONENTS_AI_CHAT_CORE_BROWSER_AI_CHAT_DRIVER_H_
...
class AIChatDriver {
 public:
  ...
  AIChatDriver(raw_ptr<PrefService> pref_service,
                        raw_ptr<AIChatMetrics> ai_chat_metrics, scoped_refptr<network::SharedURLLoaderFactory> url_loader_factory);
  ~AIChatDriver();

  AIChatDriver(const AIChatDriver&) = delete;
  AIChatDriver& operator=(const AIChatDriver&) = delete;
 ...
 private:
  ...
  base::WeakPtrFactory<AIChatDriver> weak_ptr_factory_{this};
};

}  // namespace ai_chat

#endif  // BRAVE_COMPONENTS_AI_CHAT_CORE_BROWSER_AI_CHAT_DRIVER_H_
